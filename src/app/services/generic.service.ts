...
export class GenericService<T, ID> {
  ...
  constructor(protected http: HttpClient, protected messageService: MessageService, serviceNameUrl: string) {
    this.url = 'http://insertlink' + serviceNameUrl;
  }
  ...
  getAll(): Observable<T[]> {
    return this.http.get<T[]>(this.getUrl() + ServiceEnum.ALL).pipe(
  ...
  save(t: T): Observable<T> {
    return this.http.post<T>(this.getUrl() + ServiceEnum.SAVE, t, this.httpOptions).pipe(
  ...
  edit(t: T): Observable<T> {
    return this.http.put<T>(this.getUrl() + ServiceEnum.EDIT, t, this.httpOptions).pipe(
  ...
  delete(id: ID, t: T): Observable<T> {
    return this.http.delete<T>(this.getUrl() + ServiceEnum.DELETE + '/' + id, this.httpOptions).pipe(
